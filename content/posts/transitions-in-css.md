...
subtitle: "This tutorial introduces you to CSS transitions and animations by creating an example app"
...
from basic transitions to more complex animations using keyframes 🚀
...
to enable WebP support on the flags page~~ _Update: This is not needed anymore_) and you should see
the following:

![I know this is not a perfect clone, but it will do the job 🙂](first.png)
...
<figure>
  ...
  <figcaption>The playlist is now animated</figcaption>
</figure>

Awesome, the playlist view now slides out of the bottom, by translating it 100% of its height down
the Y axis. But we _definitely_ need some transition for that, as it looks just ridiculous right
now!
...
boring, huh? Okay so let’s sprinke in a bit of animation and `@keyframes` 🙂
